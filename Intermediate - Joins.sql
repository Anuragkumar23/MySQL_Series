...
-- there are several joins we will look at, inner joins, outer joins(Left and Right), and self joins
...
JOIN employee_salary
	ON employee_data.employee_id = employee_salary.employee_id;

-- notice Ron Swanson isn't in the results? This is because he doesn't have an employee id in the data table. He refused to give his birth date or age or gender
...
-- that's because we are taking everything from the data table and only matches from the left or salary table. Since they have all the matches
-- it looks kind of like an inner join
SELECT *
FROM employee_salary sal
RIGHT JOIN employee_data ed
	ON ed.employee_id = sal.employee_id;
...
SELECT * 
FROM `employee and their data`.employee_departments;
...
INNER JOIN employee_salary sal
	ON ed.employee_id = sal.employee_id
JOIN employee_departments dept
